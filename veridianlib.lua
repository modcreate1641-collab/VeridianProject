...
if typeof(clonefunction) == "function" then
    ...
    local s, e = pcall(function() return isfolder_copy("test_shitsploit_" .. tostring(math.random(100000, 999999))) end)
    if s == false or typeof(e) ~= "boolean" then
        isfolder = function(folder) local suc, res = pcall(isfolder_copy, folder); return (if suc then res else false) end
        isfile = function(file) local suc, res = pcall(isfile_copy, file); return (if suc then res else false) end
        listfiles = function(folder) local suc, res = pcall(listfiles_copy, folder); return (if suc then res else {}) end
    end
end

Veridianhub.Toggles = {}
Veridianhub.Options = {}
Veridianhub.ConfigFolder = "VeridianConfig/Settings"

if not isfolder(Veridianhub.ConfigFolder) then
...
Veridianhub.ConfigParser = {
    Toggle = {
        ...
        Load = function(idx, data) if Veridianhub.Toggles[idx] and Veridianhub.Toggles[idx].CurrentValue ~= data.value then Veridianhub.Toggles[idx]:Set(data.value) end end
    },
    Slider = {
        Save = function(idx, obj) return { type = "Slider", idx = idx, value = obj.CurrentValue } end,
        Load = function(idx, data) if Veridianhub.Options[idx] and Veridianhub.Options[idx].CurrentValue ~= data.value then Veridianhub.Options[idx]:Set(data.value) end end
    },
    Dropdown = {
        Save = function(idx, obj) return { type = "Dropdown", idx = idx, value = obj.CurrentValue } end,
        Load = function(idx, data) if Veridianhub.Options[idx] and Veridianhub.Options[idx].CurrentValue ~= data.value then Veridianhub.Options[idx]:Set(data.value) end end
    },
    Notepad = {
        Save = function(idx, obj) return { type = "Notepad", idx = idx, text = obj.GetText and obj:GetText() or "" } end,
        Load = function(idx, data) if Veridianhub.Options[idx] and type(data.text) == "string" then Veridianhub.Options[idx]:SetText(data.text) end end
    }
}
-- =====================================================================
...
local assetsQueue = {
    ...
    {url = "https://raw.githubusercontent.com/modcreate1641-collab/Icon/refs/heads/main/aim.png",       path = iconFolder .. "/crosshair.png"},
...
for _, asset in ipairs(assetsQueue) do
    task.spawn(function()
        ...
        while not downloaded and attempts < 2 do
            ...
            if not downloaded then task.wait(0.7) end 
        end
...
function Veridianhub:CreateWindow(Config)
    ...
    local FakeNames = {
        ...
        "PerformanceStats", "DevConsoleMaster", "TouchGui", "Chat_New", "ExperienceChat", 
        "EmotesMenu", "BackpackScript", "AppLayout", "Unification", "RightClickMenu"
    }

    local RandomIndex = math.random(1, #FakeNames)
    local ChosenName = FakeNames[RandomIndex]

    local ScreenGui = Instance.new("ScreenGui", TargetGui)
    ScreenGui.Name = ChosenName 
    ...
    -- =================== UI ELEMENTS CREATE (TOGGLE BUTTON) ===================
    ...
    local function makeToggleDraggable(gui)
        ...
        gui.InputBegan:Connect(startDrag)
        for _, child in pairs(gui:GetChildren()) do
            if child:IsA("TextButton") or child:IsA("ImageButton") then child.InputBegan:Connect(startDrag) end
        end
    ...
    task.spawn(function()
        ...
        -- [[ CONTINUATION FROM PREVIOUS LOGO FOLDER CHECK ]]
    ...
    -- =====================================================================
    -- [ UI ELEMENTS : TITLE & DISCORD BUTTON ]
    -- =====================================================================
    ...
    -- =====================================================================
    -- [ UI ELEMENTS : DESTROY HUB SEQUENCE ]
    -- =====================================================================
    ...
    -- =====================================================================
    -- [ UI ELEMENTS : TOP CONFIGURATION BUTTON ]
    -- =====================================================================
    ...
    -- =====================================================================
    -- [ UI ELEMENTS : NAVIGATION SIDE FRAME ]
    -- =====================================================================
    ...
    -- =====================================================================
    -- [ UI ELEMENTS : DYNAMIC FEATURES SEARCH SYSTEM ]
    -- =====================================================================
    ...
    SearchContainer.Position = UDim2.new(0, 115, 0, 15) 
...
function TabClass:CreateLabel(cfg)
    local labelText = type(cfg) == "table" and (cfg.Text or cfg.Name or "Label") or cfg

    local frame = Instance.new("Frame", self.TabPage)
...
function TabClass:CreateButton(cfg)
    local btn = Instance.new("TextButton", self.TabPage)
...
function TabClass:CreateToggle(cfg)
...
function TabClass:CreateNotepad(cfg)
...
-- [[ 1. UPDATE WINDOWAPI:CREATETAB (เพิ่มระบบ OpenTab แบบเคลื่อนไหว & Lazy Load) ]]
function WindowAPI:CreateTab(name, target, isAuto)
    local TabPage = Instance.new("ScrollingFrame", PageArea)
    TabPage.Size = UDim2.new(1, 0, 1, 0)
    TabPage.Position = UDim2.new(0, 20, 0, 0)
...
-- [[ 2. SLIDER METHOD (OOP) ]]
function TabClass:CreateSlider(cfg)
    local dragging = false
...
-- [[ 3. DROPDOWN METHOD (OOP) ]]
function TabClass:CreateDropdown(cfg)
    local open = false
    ...
    local df = Instance.new("Frame", self.TabPage) -- อ้างอิงผ่าน self
    df.Size = closedSize
    ...
    -- [[ สร้างหน้าเซ็ตติ้งผูกไว้ใต้โครงสร้างหลัก ]]
    ...
    local function RenderSettings()
        ...
        l.TextSize = CONFIG.DefaultFontSize or 12
        l.ZIndex = 15
        ...
        title.TextColor3 = Color3.new(1, 1, 1)
        title.Font = Enum.Font.GothamBold
        title.TextSize = CONFIG.DefaultFontSize or 12
        title.TextXAlignment = Enum.TextXAlignment.Left
        title.ZIndex = 16
        ...
        -- [[ DROPDOWN BUILDER เฉพาะกิจประจำหน้า SETTING ]]
        local function BuildDrop(name, getOpts, cb)
            ...
            mb.TextColor3 = Color3.new(1, 1, 1)
            mb.Font = Enum.Font.GothamBold
            mb.TextSize = CONFIG.DefaultFontSize or 12
            mb.TextXAlignment = Enum.TextXAlignment.Left
            mb.ZIndex = 21
